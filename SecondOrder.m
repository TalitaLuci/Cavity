...
rhoprime = (12*rhos+(1-omega)*9*rhos_m02Istar)/(9+omega);
rho_m20prime = 6/5*rhos_m20Istar;
rho_m11prime = 1/3*(6*rhos_m11Istar-uLid*rhoprime);
rho_m02prime = (2*rhos+15*rhos_m02Istar)/(9+omega);
...
for i=1:9 
   for i=1:9 
    ...
    % cu = 3*(cx(i)*ux+cy(i)*uy);     
    % fEq(i,:,:) = rho .* w(i) .* ... 
    %     (1 + cu + 1/2*(cu.*cu) - 3/2*(ux.^2+uy.^2) + 27/2*ux.^2.*uy*(cx(i)^2-1/3)*cy(i)+ 27/2*uy.^2.*ux*(cy(i)^2-1/3)*cx(i)+81/4*(ux.^2.*uy.^2)*(cx(i)^2-1/3)*(cy(i)^2-1/3)); 
    % fnEq(i,:,:) = w(i) .* (9/2 * (cx(i)^2 - 1/3) * tau_xx + 9/2 * (cy(i)^2 - 1/3) * tau_yy + 9 * (cx(i) * cy(i)) * tau_xy + 27/2 * (uy .* tau_xx + 2 * ux .* tau_xy) .* (cx(i)^2 - 1/3) .* cy(i) + 27/2 * (ux .* tau_yy + 2 * uy .* tau_xy) .* (cy(i)^2 - 1/3) .* cx(i) + 81/4 * (uy.^2 .* tau_xx + 4 * ux .* uy .* tau_xy + uy.^2.*tau_yy) .* (cx(i)^2 - 1/3) .* (cy(i)^2 - 1/3));
    % fOut(i,:,:) = fEq(i,:,:)+(1-omega)*fnEq(i,:,:); 
    % fEqsec(i,:,:) = rho .* w(i) .* ... 
    %     ( 1 + cu + 1/2*(cu.*cu) - 3/2*(ux.^2+uy.^2)); 
    % fnEqsec(i,:,:) = w(i) .* (9/2 * (cx(i)^2 - 1/3) * ...
    %     tau_xx + 9/2 * (cy(i)^2 - 1/3) * tau_yy + 9 * (cx(i) * cy(i)) * tau_xy);
    % fOutsec(i,:,:) = fIn(i,:,:) - omega .* (fIn(i,:,:)-fEq(i,:,:));
   end
end
